...
void FrameHessian::makeImages(float* color, CalibHessian* HCalib)
{
	...
	{
	    //每一层影像的辐射值，x方向梯度，y方向梯度
		dIp[i] = new Eigen::Vector3f[wG[i]*hG[i]];
		absSquaredGrad[i] = new float[wG[i]*hG[i]];//梯度的赋值的平方
	}
	dI = dIp[0];//dI指向原始的图像信息
	...
	{
		int wl = wG[lvl], hl = hG[lvl];//金字塔所在层的图像尺寸
		Eigen::Vector3f* dI_l = dIp[lvl];//第lvl层的图像信息

		float* dabs_l = absSquaredGrad[lvl];////第lvl层的梯度信息
		if(lvl>0)
		{
			int lvlm1 = lvl-1;
			int wlm1 = wG[lvlm1];
